...
(function(r, n, m) {
    var p = function(e, i) {
        ...
        l.prototype = {
            ...
            destroy: function() {
                ...
                for (a = a.length; b < a; b++) {
                    this.removePrep(b);
                    for (c = 0; 0 > c; c++) this.removePrep(b + "-" + c)
                }
                this.removeCommit();
            ...
            removePrep: function(a) {
                var b, c = this.s.dt;
                if ("number" === typeof a || -1 === a.indexOf("-")) b = this.s.buttons[1 * a], b.conf.destroy && b.conf.destroy.call(c.button(a),
                    c, b, b.conf), b.node.remove(), this._removeKey(b.conf), this.s.buttons[1 * a] = null;
                else {
                    var d = a.split("-");
                    ...
                    if (h)
                        if (e.isArray(h)) this._buildButtons(h, b, c);
                            ...
                            if (j) {
                                ...
                                h.buttons && (j = this.c.dom.collection, h._collection = e("<" + j.tag + "/>").addClass(j.className), this._buildButtons(h.buttons, h._collection, f));
            ...
            _buildButton: function(a, b) {
                var c = this.c.dom.button,
                    ...
                    h =
                    function(b) {
                        return "function" === typeof b ? b(g, j, a) : b
                    };
                b && f.button && (c = f.button);
                b && f.buttonLiner && (d = f.buttonLiner);
                if (a.available && !a.available(g, a)) return !1;
                var j = e("<" + c.tag + "/>").addClass(c.className).attr("tabindex", this.s.dt.settings()[0].iTabIndex).attr("aria-controls", this.s.dt.table().node().id).on("click.dtb", function(b) {
                    b.preventDefault();
                    !j.hasClass(c.disabled) && a.action && a.action.call(g.button(j), b, g, j, a);
                    j.blur()
                }).on("keyup.dtb", function(b) {
                    b.keyCode === 13 && !j.hasClass(c.disabled) && a.action &&
                        a.action.call(g.button(j), b, g, j, a)
                });
                d.tag ? j.append(e("<" + d.tag + "/>").html(h(a.text)).addClass(d.className)) : j.html(h(a.text));
                !1 === a.enabled && j.addClass(c.disabled);
                a.className && j.addClass(a.className);
                a.namespace || (a.namespace = ".dt-button-" + s++);
                d = (d = this.c.dom.buttonContainer) ? e("<" + d.tag + "/>").addClass(d.className).append(j) : j;
                this._addKey(a);
                return {
                    node: j,
                    inserter: d
            ...
            _keypress: function(a, b) {
                ...
                c = 0;
                for (d = f.length; c < d; c++) j(f[c].conf, f[c].node);
                c = 0;
                for (d = h.length; c < d; c++) {
                    f = 0;
                    for (g = h[c].length; f < g; f++) j(h[c][f].conf, h[c][f].node)
                }
            },
        ...
        l.background = function(a, b, c) {
            c === m && (c = 400);
            a ? e("<div/>").addClass(b).css("display", "none").appendTo("body").fadeIn(c) : e("body > div." + b).fadeOut(c,
                function() {
                    e(this).remove()
                })
        };
        l.instanceSelector = function(a, b) {
            ...
            var c = [],
                ...
                f = function(a) {
                    if (e.isArray(a))
                        for (var h = 0, j = a.length; h < j; h++) f(a[h]);
        ...
        l.buttonSelector = function(a, b) {
            for (var c = [], d = function(a, b) {
                    ...
                    if (e.isArray(a) || a instanceof e) {
                        ...
                        k = a.replace(":name", "");
                        f = 0;
                        for (g = i.length; f < g; f++) i[f].name === k && c.push({
                            inst: b,
                            idx: b.toIndex(i[f].node)
                        })
        ...
        e.extend(k, {
            ...
            copy: function(a, b) {
                if (b.preferHtml && k.copyHtml5) return "copyHtml5";
        ...
        i.Api.register("button()", function(a,
            b) {
            var c = this.buttons(a, b);
            1 < c.length && c.splice(1, c.length);
            return c
        });
        i.Api.register(["buttons().active()", "button().active()"], function(a) {
        ...
        i.Api.register("buttons.exportData()", function(a) {
            if (this.context.length) {
                var b = new i.Api(this.context[0]),
                    ...
                    f = b.table().footer() ? b.columns(c.columns).indexes().map(function(a) {
                        return d(b.column(a).footer().innerHTML)
                    }).toArray() : null,
                    g = b.rows(c.rows, c.modifier).indexes().map(function(a) {
                        return b.cells(a, c.columns).indexes().map(function(a) {
                            return d(b.cell(a).render(c.orthogonal))
                        }).toArray()
                    }).toArray();
                return {
                    header: a,
    ...
    "function" === typeof define && define.amd ? define(["jquery", "datatables"], p) : "object" === typeof exports ? p(require("jquery"), require("datatables")) : jQuery && !jQuery.fn.dataTable.Buttons &&
        p(jQuery, jQuery.fn.dataTable)
})(window, document);
